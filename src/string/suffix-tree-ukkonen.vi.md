...
lang: vi
---
# Cây hậu tố (Suffix Tree). Thuật toán Ukkonen

*Bài viết này là bản nháp sơ khai và chưa chứa các mô tả chi tiết. Để tìm hiểu về thuật toán, bạn có thể tham khảo các nguồn tài liệu khác, chẳng hạn như [Algorithms on Strings, Trees, and Sequences](https://www.cs.cmu.edu/afs/cs/project/pscico-guyb/realworld/www/slidesF06/cmuonly/gusfield.pdf) của Dan Gusfield.*

Thuật toán này xây dựng một cây hậu tố (Suffix Tree) cho một xâu $s$ có độ dài $n$ trong thời gian $O(n\log(k))$, với $k$ là kích thước của bảng chữ cái (nếu $k$ được coi là hằng số thì độ phức tạp thời gian là tuyến tính).

Đầu vào của thuật toán là xâu $s$ và độ dài $n$ của nó, được truyền dưới dạng các biến toàn cục.

Hàm chính `build_tree` thực hiện việc xây dựng cây hậu tố. Cây được lưu trữ dưới dạng một mảng các cấu trúc `node`, trong đó `node[0]` là gốc của cây.

Để đơn giản hóa mã nguồn, các cạnh được lưu trữ trong cùng các cấu trúc này: với mỗi đỉnh, cấu trúc `node` của nó lưu trữ thông tin về cạnh nối giữa đỉnh đó và nút cha của nó. Nhìn chung, mỗi `node` lưu trữ các thông tin sau:

* `(l, r)` - biên trái và biên phải của xâu con `s[l..r-1]` tương ứng với cạnh dẫn đến nút này,
* `par` - nút cha,
* `link` - liên kết hậu tố (suffix link),
...
Bản cài đặt rút gọn này được đề xuất bởi [freopen](http://codeforces.com/profile/freopen).
...
Đoạn mã tương tự có kèm chú thích:
